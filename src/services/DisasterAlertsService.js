...
class DisasterAlertsService {
  ...
  async getAllDisasterAlerts(lat = 14.5995, lon = 120.9842) {
    // Default to Manila
    try {
      const [earthquakes, tsunamiAlerts, pagasaAlerts, naturalEvents] =
      ...
      return {
        ...
        tsunami:
          tsunamiAlerts.status === "fulfilled" ? tsunamiAlerts.value : [],
      ...
      const tsunamiAlerts = [
        {
          ...
          description:
            "Magnitude 6.7 earthquake offshore Cebu may cause minor sea-level disturbance. Monitor coastal areas.",
          severity: "moderate",
          ...
          source: "PHIVOLCS",
          advisory:
            "Minor sea-level disturbance expected. Coastal monitoring advised.",
        },
      ];

      return tsunamiAlerts.filter((alert) =>
        this.isWithinPhilippines(alert.coordinates.lat, alert.coordinates.lon)
      );
    } catch (error) {
  ...
  async getEarthquakeAlerts() {
    try {
      ...
      const phivolcsEarthquakes = [
        {
          ...
          description:
            "Strong earthquake detected offshore Northern Cebu. Monitor for aftershocks.",
          severity: "critical",
        ...
        },
        {
          ...
          description:
            "Moderate earthquake northwest of Currimao, Ilocos Norte.",
          severity: "moderate",
      ...
      const usgsResponse = await fetch(
        `${this.USGS_BASE_URL}/significant_week.geojson`
      );
      ...
      const pagasaAlerts = [
        {
          ...
          description:
            "Monitor approaching weather disturbance. Heavy rainfall expected in Luzon areas.",
          severity: "moderate",
          startTime: new Date().toISOString(),
          location: "Luzon",
          source: "PAGASA",
          advisory:
            "Heavy rainfall and strong winds expected. Monitor local advisories.",
        },
      ];
  ...
  async getNaturalEventAlerts() {
    try {
      const response = await fetch(
        `${this.NASA_EONET_URL}?status=open&limit=50`
      );
      ...
      const relevantEvents = data.events.filter((event) => {
        ...
        return event.geometry.some((geo) => {
          if (geo.coordinates) {
            ...
            return (
              this.isWithinPhilippines(lat, lon) ||
              this.isGlobalImpact(event.categories)
            );
          }
          return false;
        });
  ...
  isWithinPhilippines(lat, lon) {
    return (
      lat >= this.PHILIPPINES_BOUNDS.south &&
  ...
  getWeatherSeverity(eventType) {
    ...
    if (moderateSeverity.some((type) => event.includes(type)))
      return "moderate";
    return "low";
  }
